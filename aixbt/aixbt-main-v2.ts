...
class JobProcessor {
    ...
    private async handleJob(job: AcpJob, memoToSign?: AcpMemo) {
        try {
            ...
            // Check if we can get the agent's jobs to compare
            try {
                ...
                if (agent) {
                    ...
                    // Find which job matches this job
            ...
            if (
                ...
                try {
                    ...
                    if (is404Error || isIndigoError) {
                        // Reject the job for API failures
                        console.log(`[REJECT] Rejecting job ${job.id} due to service failure`);
                        await job.reject(`Service API failure: ${errorMessage}`);
